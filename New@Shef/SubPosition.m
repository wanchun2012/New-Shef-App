...
-(void)initDB
{
    ...
    if ([filemgr fileExistsAtPath: databasePath ] == NO)
    {
        ...
        NSLog(@"DB:Subposition: Sqlite3: create one database in document directory");
    }
    else
    {
        NSLog(@"DB:Subposition: Sqlite3: exist one database in document directory");
    }
}
...
- (void) clearData
{
    ...
    if (sqlite3_open(dbpath, &db) == SQLITE_OK)
    {
        ...
        {
            NSLog(@"DB:Subposition: Sqlite3: Fail delete, qlite3_prepare_v2()");
    ...
    {
        NSLog(@"DB:Subposition: Sqlite3: Fail insert, fail open db");
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Fail load data, Exit app?" delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil];
        [alert show];
    }
}
...
- (NSArray *) selectData
{
    ...
    if (sqlite3_open(dbpath, &db) == SQLITE_OK)
    {
        ...
        if(sqlite3_prepare_v2(db, select_stmt,-1, &statement, NULL)==SQLITE_OK)
        {
            ...
            {
                NSLog(@"DB:Subposition: Sqlite3: Success select");
        ...
        {
            NSLog(@"DB:Subposition: Sqlite3: Fail select, qlite3_prepare_v2()");
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Fail load data, Exit app?" delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil];
            [alert show];
        }
...
- (void) saveData:(int)Id name:(NSString *)n
          uebName:(NSString *)un contenttype:(NSString *)t imageURL:(NSString *)url foreignkey:(int)fk
{
    sqlite3_stmt *statement;
    const char *dbpath = [databasePath UTF8String];
    ...
    if (sqlite3_open(dbpath, &db) == SQLITE_OK)
    {
        ...
        {  
            ...
            {
                NSLog(@"DB:Subposition: Sqlite3: Success insert");
            }
            else
            {
                NSLog(@"DB:Subposition: Sqlite3: Fail insert, sqlite3_step()");
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Fail load data, Exit app?" delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil];
                [alert show];
            }
        }
        ...
        {
            NSLog(@"DB:Subposition: Sqlite3: Fail insert, qlite3_prepare_v2()");
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Fail load data, Exit app?" delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil];
            [alert show];
        }
        sqlite3_clear_bindings(statement);
...
- (void) updateData:(int)Id text:(NSString *)txt
{
    ...
    if (sqlite3_open(dbpath, &db) == SQLITE_OK)
    {
        ...
        if(sqlite3_prepare_v2(db, update_stmt,-1, &statement, NULL)==SQLITE_OK)
        {
            ...
            if (sqlite3_step(statement) == SQLITE_DONE)
            {
                NSLog(@"DB:Subposition: Sqlite3: Success update");
            }
            else
            {
                NSLog(@"DB:Subposition: Sqlite3: Fail update, sqlite3_step()");
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Fail load data, Exit app?" delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil];
                [alert show];
            }
        }
        ...
        {
            NSLog(@"DB:Subposition: Sqlite3: Fail update, qlite3_prepare_v2()");
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Fail load data, Exit app?" delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil];
            [alert show];
        }
